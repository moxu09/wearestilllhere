...
export async function createPlatformPayment(orderId: string, accessToken: string, siteOrigin: string) {
  if (!getEcpayConfig().inSiteAvailable) throw new Error("綠界站內刷卡尚未開放");
  ...
  if (order.payment_method !== "card" || order.payment_status !== "unpaid" ||
    ...
    throw new Error("這筆訂單不能再次刷卡付款");
  const amount = Number(order.total_amount);
  if (!Number.isSafeInteger(amount) || amount < 6 || amount > 199_999)
    throw new Error("站內刷卡金額須介於 NT$6 至 NT$199,999");
  ...
  const { data: existing, error: existingError } = await admin.from("ecpay_platform_payments")
    .select("merchant_trade_no,status").eq("platform_order_id", orderId).maybeSingle();
  if (existingError) throw new Error("無法確認網站付款單");
  if (existing) {
    if (existing.status !== "pending") throw new Error("這筆訂單已付款，請重新整理訂單頁");
    return { insiteUrl: `/payments/ecpay/platform/insite?order=${encodeURIComponent(existing.merchant_trade_no)}` };
  }
  ...
  const { error: insertError } = await admin.from("ecpay_platform_payments").insert({
    ...
    customer_user_id: user.id, amount, site_origin: siteOrigin,
  });
  if (insertError) {
    if (insertError.code !== "23505") throw new Error("無法建立網站付款單");
    const { data: raced } = await admin.from("ecpay_platform_payments")
      .select("merchant_trade_no,status").eq("platform_order_id", orderId).maybeSingle();
    if (!raced || raced.status !== "pending") throw new Error("付款單已建立，請回到訂單頁查詢");
    return { insiteUrl: `/payments/ecpay/platform/insite?order=${encodeURIComponent(raced.merchant_trade_no)}` };
  }
  return { insiteUrl: `/payments/ecpay/platform/insite?order=${merchantTradeNo}` };
}
